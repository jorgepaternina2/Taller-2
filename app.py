...
# Cambio para GitHub
# Esta linea fue agregada por mi compañero
...
def load_data():
    ...
    # Asumimos que es la primera columna del archivo (índice 0).
    nombre_columna_fecha = df.columns[0]
    df[nombre_columna_fecha] = pd.to_datetime(df[nombre_columna_fecha])
    
    # 3. Asignar la fecha como índice (Requisito del PDF)
    df.set_index(nombre_columna_fecha, inplace=True)
